        ...
        <div class="col-lg-12">
            <div class="ibox ">
                ...
                <div class="ibox-content">
                    <form method="POST" action="{{ route('roles.update', $rolModel) }}" id="form_edit_role">
                        {{ method_field('PUT') }}
                        {{ csrf_field() }}
                        ...
                        <div class="form-group  row"><label class="col-sm-2 col-form-label">Nombre</label>
                            ...
                            <div class="col-sm-10">
                                @if($rolModel->name == 'superadmin')
                                    <p class="bg-primary">{{ $rolModel->name }}</p>
                                    <input type="hidden" class="form-control" name="name" id="name" value="{{ $rolModel->name }}">
                                @else
                                    <input type="text" class="form-control" name="name" id="name" value="{{ old('name', $rolModel->name) }}">
                                @endif
                                <input type="hidden" name="id_rol" value="{{ $rolModel->id }}">
                                    @if($errors->has('name'))
                        ...
                        <div class="form-group  row"><label class="col-sm-2 col-form-label">Título</label>
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" name="title" id="title" value="{{ old('title', $rolModel->title) }}">
                                @if($errors->has('title'))
                        ...
                        <div class="form-group  row"><label class="col-sm-2 col-form-label">Descripción</label>
                            <div class="col-sm-10">
                                <textarea class="form-control" cols="100" rows="5" name="description">{{ old('description', $rolModel->description) }}</textarea>
                                        ...
                                        @foreach($modelPermisos as $mp)
                                            ...
                                            <div class="row">
                                                @foreach($mp['permisos'] as $permiso)
                                                    <div class="col"><label> <input type="checkbox" name="permisos[]" value="{{ $mp['model'].'/'.$permiso }}"
                                                                                    @if($rolModel->name == 'superadmin')
                                                                                        checked="" disabled=""
                                                                                    @else
                                                                                        @foreach($mp['autorizados'] as $autorizado)
                                                                                            @if($autorizado === $permiso)
                                                                                                checked=""
                                                                                            @endif
                                                                                        @endforeach
                                                                                    @endif
                                                            > <i></i> {{ $mp['model'] }} {{ $permiso }} </label></div>
                                                    <?php $i++; ?>
                                                @endforeach
                                                <?php while($i < 4){ ?>
                                                    <div class="col"></div>
                        ...
                        @if(count($funciones) > 0)
                            ...
                            <div class="form-group row"><label class="col-sm-2 col-form-label">Permisos por funciones<br/>
                                ...
                                <div class="col-sm-10">
                                    ...
                                    <div class="row">
                                        <?php } ?>
                                        <div class="col"><label> <input type="checkbox" name="funciones[]" value="{{ $funcion }}"
                                                                        @if($rolModel->name == 'superadmin')
                                                                            checked="" disabled=""
                                                                        @else
                                                                            @foreach($abilities as $ability)
                                                                                @if($ability->name === $funcion)
                                                                                    checked=""
                                                                                @endif
                                                                            @endforeach
                                                                        @endif
                                                > <i></i> {{ $funcion }} </label></div>
                                        <?php $i++;?>
                                        @endforeach
                                        <?php while($i%4 != 0){ ?>
                                        <div class="col"></div>
